...
public class Infantry_Model : MonoBehaviour
{
    [Header("Enemey Stats")]
    ...
    [SerializeField] private Rifle_Model weapon;
    ...
    [SerializeField] public int moveDirection = -1; // +1 = right, -1 = left
    ...
    private void Update()
    {
        ...
        {
            animator.SetBool("running", false); // Stop
            animator.SetTrigger("shooting");    // Shoot animation
            enemyBoby.linearVelocity = Vector2.zero;

            Flip(true); // Face the target
        }
        else
        {
            animator.ResetTrigger("shooting");
            animator.SetBool("running", true);  // Move
            enemyBoby.linearVelocity = new Vector2(moveSpeed * moveDirection, enemyBoby.linearVelocity.y);

            Flip(false); // Face running direction
        }
    }
    ...
    private void Flip(bool faceTarget)
    {
        if (faceTarget && targetAllied != null)
        {
            float directionToTarget = targetAllied.transform.position.x - transform.position.x;
